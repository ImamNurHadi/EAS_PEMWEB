    ...
    <div v-if="showAddRegistrationForm" class="bg-gray-700 p-4 rounded-lg shadow-lg mb-4">
      <form @submit.prevent="submitRegistrationForm" class="custom-form">
        <div class="mb-4">
          ...
          <input v-model="newRegistration.nama" type="text" id="Nama"
            class="form-input mt-1 block w-full rounded-md pl-3 text-black">
        </div>
        <div class="mb-4">
          <label for="Email" class="block text-sm font-medium">Email</label>
          <input v-model="newRegistration.email" type="email" id="Email"
            class="form-input mt-1 block w-full rounded-md pl-3 text-black">
        </div>
        <div class="mb-4">
          <label for="AsalSekolah" class="block text-sm font-medium">Asal Sekolah</label>
          <input v-model="newRegistration.asal_sekolah" type="text" id="AsalSekolah"
            class="form-input mt-1 block w-full rounded-md pl-3 text-black">
        </div>

        <button type="button" @click="cancelForm"
          class="bg-red-500 hover:bg-red-600 text-white font-semibold px-4 py-2 rounded-md ml-2">Batal</button>
        <button type="submit"
          class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 rounded-md ml-2">Daftar</button>
      </form>
    </div>
...
export default {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/pendaftaran', {
      ...
      body: JSON.stringify({
        nama: this.newRegistration.nama,
        email: this.newRegistration.email,
        asal_sekolah: this.newRegistration.asal_sekolah,
    ...
    if (response.ok) {
      ...
      console.error('Gagal mendaftar. Status:', response.status, errorData);
    }
  } catch (error) {
